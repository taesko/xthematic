...
from xredit.term import TERMINAL_COLORS


class ColoredContext:
    all_color_identifiers = set(xredit.colors.ColorIdentifier.all_four_bit_colors())
    ...
    def register_color(self, color):
        ...
        elif color in xredit.term.TERMINAL_COLORS.values():
            raise ValueError(f"color {color} is already defined in the terminal's colors")

        id_ = self.unregistered_ids.pop()
        self.overwritten_colors[id_] = xredit.term.TERMINAL_COLORS[id_]
        xredit.term.TERMINAL_COLORS[id_] = color

    def unregister_color(self, color):
        id_ = self.id_for_color(color)
        xredit.term.TERMINAL_COLORS[id_] = self.overwritten_colors[id_]
    ...
    def printable_colors():
        return xredit.term.TERMINAL_COLORS.values()

    @staticmethod
    def id_for_color(color):
        for id_, value in xredit.term.TERMINAL_COLORS.items():
            if value == color:
                return id_
...
def echo_theme(theme_name=None):
    if theme_name:
        colors = xredit.themes.theme_colors(theme_name)
    else:
        colors = xredit.term.TERMINAL_COLORS
    with ColoredStream.open() as stream:
        for row_id in xredit.colors.ColorIdentifier.all_four_bit_colors():
            stream.echo(text=row_id, fg=colors[row_id], nl=True)
            # for col_id in list(xredit.colors.ColorIdentifier.all_four_bit_colors())[:8]:
